...
- define R_NO_REMAP and switch to Rf\_\* functions at the request of
  CRAN by [@jefferis](https://github.com/jefferis) in
  <https://github.com/jefferislab/RANN/pull/32>
- Use namespacing to isolate ann library and vendor source by
  [@krlmlr](https://github.com/krlmlr) in
  <https://github.com/jefferislab/RANN/pull/30> (this could be used to
  provide the L1 metric in the same package in future)
- switch to github actions by [@jefferis](https://github.com/jefferis)
  in <https://github.com/jefferislab/RANN/pull/33>
...
- Fix Solaris compile error (as requested by BDR,
  [\#25](https://github.com/jefferislab/RANN/issues/25))
- Correct usage for compiler vs preprocessor flags
  ([\#26](https://github.com/jefferislab/RANN/issues/26))

## RANN 2.6.0

- remove register key word from libANN code (at request of CRAN,
  [\#23](https://github.com/jefferislab/RANN/issues/23))
...
- bug fix wrong result dimensions when query is a vector
  ([\#17](https://github.com/jefferislab/RANN/issues/17)) thanks to
  github user dayey1
...
- fix crashing bug when receving N x 0 input matrix (thanks to Rajaraman
  V for bug report)
...
CRAN release: 2013-07-24

- remove nn() function since underlying get_NN() had memory leaks
  (noticed by Brian Ripley)
- make nn2() use input data as query when query not specified)
- add Arya and Mount as authors (in accordance with CRAN policies, again
  BDR)
- add Arya, Mount and University of Maryland in Copyright line (in
  accordance with CRAN policies, again BDR)
- add COPYRIGHT file to make it clear exactly which files are from the
  ANN library and therefore subject to that library’s copyright.

## RANN 2.2.1

- fix yet more warnings for CRAN due to presence in ANN of exit(),
